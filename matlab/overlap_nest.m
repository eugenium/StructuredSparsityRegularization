...
while (t < iters_acc && abs(prev_cost - curr_cost)/curr_cost > eps_acc)
    if(mod(t,verbosity)==0)
        disp(['overlap_nest - iteration number '  num2str(t) ' and current cost ' num2str(curr_cost) ' epsilon accuracy ' num2str(abs(prev_cost - curr_cost))]);
    end
