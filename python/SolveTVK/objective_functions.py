...
def tv_ksp_from_gradient(gradient,k,square=False):
    ...
    tv_term = np.sum(np.sqrt(np.sum(gradient[:-1] * gradient[:-1],
                                    axis=0)))
    if(square):
        k_term = KSup.f(gradient[-1].ravel(),k)**2
    else:
        k_term = KSup.f(gradient[-1].ravel(),k)
    return k_term + tv_term

def div_id(grad, l1_ratio=.5,square=False,fact=1):
    ...
    # the identity part
    if(square):
        res-= np.sqrt(l1_ratio)* grad[-1]     
    else:
        res -= l1_ratio * grad[-1]*fact

    return res


def gradient_id(img, l1_ratio=.5,square=False,fact=1):
    ...
    # the identity part
    if(square):
        gradient[-1] = np.sqrt(l1_ratio) * img
    else:
        gradient[-1] = l1_ratio * img*fact
